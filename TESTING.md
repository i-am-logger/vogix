...
| `openrgb-owner` | `vogix machine serve openrgb` as a hardened `vogix-openrgb.service` (DynamicUser, loopback only; `BindsTo=`/`Upholds=` from `openrgb.service`) against that real server with Debug DRAM and keyboard, DDP and Govee devices, following a `palette.json` published into the drop zone. The colours are observed on OpenRGB's own DDP datagrams and Govee commands, not through vogix's decoder: the palette applied while the server detects, every selected controller confirmed at protocol 6 in `status.json` and `STATUS=`, and `vogix machine status` reading it and exiting 0; the absent device warned about once after `DETECTION_COMPLETE`; a theme change and five rapid ones land as the last; SIGHUP re-applies; `vogix machine inspect` lists the controllers and captures their raw payloads at protocols 6 and 5 (`$out/capture`, the source of `tests/fixtures/openrgb`); a restarted openrgb stops the owner cleanly and a new one re-confirms; a killed openrgb makes the owner exit 75 and both come back; with `maxProtocol` 5 (a specialisation switch) the owner speaks protocol 5 and the colours land again. |
| `machine-local` | `vogix machine serve local` under a unit written as `vogix-machine.service` runs it (`Type=notify`, root with `CAP_SYS_TTY_CONFIG` only, a closed device policy allowing `/dev/tty0` and hidraw, a private network with `AF_UNIX`/`AF_NETLINK`), with the console on and one command device (a recorder re-run on hidraw `0627:0001`), fed by the declared owner's real CLI. Before any publish the owner is ready and waiting; `vogix theme set` publishes after the state commit and the kernel's VT palette (`/sys/module/vt/parameters/default_{red,grn,blu}`) follows the theme package's `console/palette` with no VT switch; the command device runs with its slot's colour; identical bytes are not republished; another user's apply does not reach the machine; a hot-added QEMU `usb-kbd` hidraw node re-runs the device; SIGHUP re-runs the devices; a palette the drop zone's owner did not write is rejected and `vogix machine status` exits 1 naming it, until the owner's `vogix theme refresh` replaces it; after a reboot the palette is applied before `systemd-user-sessions`. |
