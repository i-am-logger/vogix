...
mod tests {
    ...
    fn test_pane_launch_always_has_cwd() {
        let titles = ["btop", "lumen", "claude", "hx", "bash", "unknown", "", "ζ€ε₯‡ηš„η¨‹εΊ"];
        for title in &titles {
            let pane = WeztermPane {
                pane_id: 0,
                title: title.to_string(),
            ...
            let (cmd, args) = pane_launch_command(&pane);
            assert_eq!(cmd, "wezterm");
            assert!(args.contains(&"--cwd".to_string()), "Missing --cwd for title: {}", title);
            assert!(args.contains(&"/home/user/".to_string()), "Missing dir for title: {}", title);
        }
    }
    ...
    fn test_cli_parse_restore_with_json() {
        use crate::cli::Cli;
        use clap::Parser;
        let cli = Cli::try_parse_from(["vogix", "session", "restore", "--json", "/tmp/test.json"]).unwrap();
        if let crate::cli::Commands::Session { command: crate::cli::SessionCommands::Restore { json, dry_run, .. } } = cli.command {
    ...
    fn test_cli_parse_restore_with_dry_run() {
        ...
        if let crate::cli::Commands::Session { command: crate::cli::SessionCommands::Restore { dry_run, .. } } = cli.command {
            assert!(dry_run);
        } else {
            panic!("Expected Session Restore");
        }
    ...
    fn test_cli_parse_restore_json_and_dry_run() {
        use crate::cli::Cli;
        use clap::Parser;
        let cli = Cli::try_parse_from(["vogix", "session", "restore", "--json", "/tmp/x.json", "--dry-run"]).unwrap();
        if let crate::cli::Commands::Session { command: crate::cli::SessionCommands::Restore { json, dry_run, .. } } = cli.command {
