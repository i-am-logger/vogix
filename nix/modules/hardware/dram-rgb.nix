...
in
{
  config = mkIf cfg.enable {
    ...
    # this module -- not the keyboard -- owns the i2c side of the OpenRGB
    # server that its OpenRGB device enables.
    #
    # Every ENE DRAM controller (one per stick) shows the slot's colour in
    # its Static mode, set by vogix-openrgb.service.
    vogix.hardware.devices.dram-rgb = {
      slot = cfg.colorSlot;
      provider.openrgb = {
        nameContains = "ENE DRAM";
        mode = "Static";
      };
    };
