...
// screen and something is playing, stopped (and the bars zero-filled)
// the moment either ends. The process is supervised by AudioTap: it
// waits for PipeWire and a default sink, and comes back after an exit
// nobody asked for. Config is inlined over stdin — no file to manage.
// Frame writes skip when nothing changed, and the ballistics tick stops
// once every bar has settled, so silence costs no repaints and no
// wakeups.
import QtQuick
import Quickshell
...
Singleton {
    ...
    property var _cap: []
    property var _hold: []
    // Every bar sits on the current frame: nothing moves until a new one.
    ...
    // A new frame unsettles it; it stops again once everything has landed.
    Timer {
        interval: 40   // cava's own 25 fps
        running: root.active && !root._settled
        repeat: true
    ...
    function _advance(dt: real): void {
        ...
        root._settled = Ballistics.settled(src, root._level, root._cap);
    }

    // A new frame: something may move.
    function _frame(frame: var): void {
        root.values = frame;
        root._settled = false;
    }
    ...
    AudioTap {
        ...
        // A stopped tap must not leave its last frame standing: zero it and
        // let the ballistics fall.
        onRunningChanged: {
            if (!running) {
                root._frame(Array(root.bars * 2).fill(0));
                root._last = "";
            }
        }
        ...
        onLine: data => {
            if (data === root._last)
                return;
            root._last = data;
            root._frame(data.split(";").filter(s => s !== "")
